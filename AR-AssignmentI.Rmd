...
# Transform the nir_spectra using the B-spline basis functions
basis_expansion <- function(data, basis) {
  n <- ncol(data)
  expanded_data <- matrix(NA, nrow = nrow(data), ncol = n * ncol(basis))
  for (i in 1:n) {
    expanded_data[, ((i - 1) * ncol(basis) + 1):(i * ncol(basis))] <- data[, i] %*% t(basis)
  }
...
# Fit a linear model using the coefficients
lm_model <- lm(octane ~ nir_fd_coefs - 1)  # Remove intercept to match manual prediction
...
octane_pred_manual <- nir_fd_coefs %*% fitted_coefficients
