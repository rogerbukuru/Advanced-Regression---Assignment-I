...
apply_wavelet <- function(data, n_coef = 256) {
  # Find the next power of 2 that's at least as large as the data length
  next_power_of_2 <- 2^ceiling(log2(length(data)))
  
  # Pad the data with zeros to reach the next power of 2
  ...
  # Collect coefficients from all levels
  all_coefs <- unlist(lapply(all_levels, function(l) accessD(wt, level = l)))
  
  # Return the first n_coef coefficients (or all if less than n_coef)
...
coef_importance <- abs(coef(model))[-1]  # Exclude intercept
...
plot(coef_importance, main = "Wavelet Coefficient Importances", 
     xlab = "Coefficient Index", ylab = "Absolute Coefficient Value")
points(top_features, coef_importance[top_features], col = "red", pch = 19)


coefficients <- coef(model)

# Extract and plot the non-zero coefficients
non_zero_coeffs <- coefficients[coefficients != 0]
